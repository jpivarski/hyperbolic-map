...
// centre is in the group and its image swaps them.

// ---- the palette -------------------------------------------------------------------------------
//
// Jim's shades, taken from the four fish fills in escher-atlas.json. Index order matters: 0 and 1 are
// the pair the prototype octagon shows, so `stabiliser` below swaps 0 with 1 and 2 with 3.
...
// specification of the centre octagon -- green at top-left and bottom-right, yellow at top-right and
// bottom-left -- puts role 0 (yellow) on the (63, -118) diagonal and role 1 (green) on the (151, -27)
// one. Each fish is drawn as two shapes, one through the tile centre and one further out, hence two
// entries per fill.
...
// ---- the overlap pieces --------------------------------------------------------------------------
//
// Four shapes in the file, drawn in four greys, are not this octagon's fish at all: they are the parts
// of four NEIGHBOURS' fish that fall inside this octagon, which the tile has to draw itself because
// each tile is clipped to its own boundary.
//
// So their roles are not free -- each is whatever color that neighbour paints that fish, which is
// `neighbour's permutation applied to that fish's role`. Recorded here as (which neighbour, which role
// in that neighbour) and resolved against the live tiling by `overlapRoles` below, so that changing PHI
// cannot leave a stale constant behind.
//
// Established by mapping every neighbour's shapes through `stepFrame` into this tile's frame and taking
// the directed point-to-polyline distance: each grey matches one shape at 0.04 against a 0.21
// runner-up, so there is no ambiguity about which fish it is a piece of.
export const OVERLAPS = [
  { fill: "#999999", generator: 0, role: 0 }, // neighbour at    0 deg, its -118 fish
  { fill: "#cccccc", generator: 4, role: 0 }, // neighbour at  180 deg, its   63 fish
  { fill: "#e6e6e6", generator: 2, role: 1 }, // neighbour at   90 deg, its  151 fish
  { fill: "#b3b3b3", generator: 6, role: 0 }, // neighbour at  -90 deg, its   63 fish
];

// ---- the homomorphism ------------------------------------------------------------------------------
//
// PHI_STABILISER is the image of P, the 90-degree rotation about a tile centre. It has to swap the two
...
export const PHI_STABILISER = [1, 0, 3, 2];
...
//
// Measure the whole area, not sampled fish centres. Scoring only the centre of each fish ranked a
...
export function escherColorSymmetry() {
  ...
  for (let g = 0; g < 6; g++) {
    gens[g + 2] = compose(compose(PHI_STABILISER, gens[g]), inverse(PHI_STABILISER));
  }
  return { colors: PALETTE.length, generators: gens, stabiliser: PHI_STABILISER };
}

// The role each overlap piece plays in the PROTOTYPE tile, asked of the tiling rather than written down:
// it is the neighbour's own permutation applied to the fish's role there. Returns fill -> role, ready to
